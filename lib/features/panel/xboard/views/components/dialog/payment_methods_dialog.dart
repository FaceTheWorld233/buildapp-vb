...
class _PaymentMethodsDialogState extends ConsumerState<PaymentMethodsDialog> {
  ...
  Widget build(BuildContext context) {
    ...
    final userInfo = accountBalanceViewModel.userInfo; // 获取用户余额信息
    final t = ref.watch(translationsProvider); // 引入本地化文件
    final userBalance = (userInfo?.balance ?? 0) / 100; // 转为元单位
    return AlertDialog(
      title: Text(
        ...
        child: Column(
          ...
          children: [
            ...
            ...widget.paymentMethods.map((method) {
              ...
              // 减去余额后的总价，确保不小于零
              final totalPrice =
                  (widget.totalAmount + handlingFee - userBalance)
                      .clamp(0, double.infinity);
              final count = widget.totalAmount-totalPrice;
              return Column(
                children: [
                  ListTile(
                    title: Text(
                    ...
                    subtitle: Padding(
                      ...
                      child: Column(
                        ...
                        children: [
                          ...
                          Text(
                            '${t.purchase.totalPrice}: ${totalPrice.toStringAsFixed(2)} ${widget.t.purchase.rmb}(${t.userInfo.balance}-$count)',
                            style: const TextStyle(
                              fontSize: 18,
                              fontWeight: FontWeight.bold,
                              color: Colors.blueAccent,
