...
class PerAppProxyList extends _$PerAppProxyList {
  ...
  Future<void> excludeCurrentApp() async {
    final currentAppPackage = await PlatformUtils.getCurrentAppPackageName(); // 获取应用包名
    final updatedList = [...state];

    if (!updatedList.contains(currentAppPackage)) {
      updatedList.add(currentAppPackage); // 将当前应用包名加入排除列表
    }

    state = updatedList;
    await _exclude.write(updatedList); // 更新 SharedPreferences
  }
}
